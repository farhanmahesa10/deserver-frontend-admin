...
export default function Event() {
  ...
  //function mengambil data event
  const fetchData = async () => {
    try {
      // Mengambil data transaksi menggunakan axios dengan query params
      const response = await axios.get(
        ` ${process.env.NEXT_PUBLIC_BASE_API_URL}/api/v1/event/showcafename/${outletName}`
      );
  ...
  const handleRemove = async (dataRemove) => {
    ...
    try {
      ...
      const response = await axios.delete(
        `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/v1/event/delete/${dataRemove}`,
